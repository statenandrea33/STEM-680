...
naep_data_clean <- read.csv("naep-data.csv") %>% # Read in the naep-data.csv
...
```{r}
naep_data_clean %>% 
    pull(average_score_0_500) # Extract the `average_score_0_500` column
```
...
naep_data_clean %>%
    filter(jurisdiction == "Illinois") %>%
    select(jurisdiction, average_score_0_500) # Keep only jurisdiction and average score columns
```
...
* The state with the minimum average score is `r naep_data_clean %>% slice_min(average_score_0_500) %>% pull(jurisdiction)` with an average score of `r naep_data_clean %>% slice_min(average_score_0_500) %>% pull(average_score_0_500)`.
...
* The average score for my home state of `r naep_data_clean %>% filter(jurisdiction == "Tennessee") %>% pull(jurisdiction)` is `r naep_data_clean %>% filter(jurisdiction == "Tennessee") %>% pull(average_score_0_500)`.
...
* `r naep_data_clean %>% filter(average_score_0_500 > 230) %>% nrow()`
...
A negative sign before a column name will exclude this column in the selection.

```{r}
naep_data_clean %>% 
  filter(average_score_0_500 > 230) %>% 
  select(-at_or_above_basic)
```
