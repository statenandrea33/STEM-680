...
```{r}
elsi_data <- read_csv("ELSI_csv_export.csv", skip = 6)
...
- The data is looking at schools in Tennessee.
- It looks like it provides a lot of information on student to teacher ratios.
...
```{r}
elsi_grouped %>% 
  summarise(mean_students = mean(total_students_all_grades_excludes_ae_public_school_2021_22))
```
...
```{r}
elsi_grouped2 <- elsi_data %>% 
  group_by(locale_public_school_2021_22) %>% 
  summarise(mean_students = mean(total_students_all_grades_excludes_ae_public_school_2021_22))
```
...
```{r}
elsi_groupmultiple <- elsi_data %>% 
  group_by(national_school_lunch_program_public_school_2021_22, agency_type_district_2021_22) %>% 
  summarise(mean_students = mean(total_students_all_grades_excludes_ae_public_school_2021_22)) %>% 
  arrange(mean_students)
```
...
```{r}
elsi_final <- elsi_data %>% 
  group_by(agency_id_nces_assigned_public_school_latest_available_year, locale_public_school_2021_22) %>% 
  summarise(mean_students = mean(total_students_all_grades_excludes_ae_public_school_2021_22), min_students = min(total_students_all_grades_excludes_ae_public_school_2021_22),
            max_students = max(total_students_all_grades_excludes_ae_public_school_2021_22)) %>% 
  arrange(mean_students)
...
- What I noticed is most schools with smaller student bodies were in rural areas, while cities or towns tended to have larger student bodies. This makes sense, after looking at a few of the outliers of schools in cities with small student bodies, both of the ones I selected were schools for the blind and deaf.
