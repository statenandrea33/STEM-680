...
Next, let's load two files, one for the data on nurses (from the [*National Teacher and Principal Survey*](https://nces.ed.gov/surveys/ntps/)) and the other a simple data frame with the names of U.S. states and their region. For data access, I uploaded them in Canvas.

```{r}
dat <- read_csv("ntps2021_7341_s1s.csv") # data on nurses in U.S. schools, by state
dat

state_by_region <- read_csv("us census bureau regions and divisions.csv") # data on U.S. states' regions
```
## Step 3
...
```{r}
dat2 <- read.csv("ntps2021_7341_s1s.csv", skip = 7)
dat2
```
...
```{r}
dat2 <- dat2 %>% 
  select(State = State, avg_nurses = 2, students_per_nurse = 4)
```
...
```{r}
joined_data <- inner_join(dat4, state_by_region, by = c("State"))
```
...
grouped_data <- joined_data %>% 
  group_by(Region) %>% 
  summarise(
    mean_avgnurses = mean(variable == "avg_nurses"),
    mean_studentspernurse = mean(variable == "students_per_nurse"),
    sd_avgnurses = sd(variable == "avg_nurses"),
    sd_studentspernurse = sd(variable == "students_per_nurse")
  )
```

What do you notice about any differences by region? Add a note or two below.

- The differences by region are very minimal.
...
- The data has a slightly bigger difference between divisions than regions did.
