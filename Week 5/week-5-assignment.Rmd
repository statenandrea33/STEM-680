...
```{r}
tidykids <- read.csv("tidykids.csv")
naep <- read.csv("naep-data.csv")
```
...
```         
- Both datasets are looking at data for entire states, but the tidykids data is looking at different education levels.
```
...
- Each variable is not in a column, this is why there are so many rows!
...
- The NAEP dataset has data for the District of Columbia, but the tidykids dataset does not.
...
```{r}
naep <- rename(naep, state = Jurisdiction)
```
...
-   The order of columns is different in both dataframes, one has the tidykids columns first, the other has the naep.

-   The number of rows is different, one has 50 and the other has 51.
...
```{r}
joined_left <- clean_names(joined_left)
```
...
```{r}
joined_left %>% 
  filter(state %in% c("Alabama", "Florida", "Kentucky", "Maryland", "New Jersey", "Tennessee", "Texas", "Vermont", "Virginia", "Washington")) %>% 
  ggplot(aes(x = average_score_0_500, y = highered, color = state, group = state))+
  geom_point() +
...
-   It seems like on average, most states that were selected that scored in between 232-236, spent more on higher education than other states with different score ranges.

- This analysis could be extended to look at how this amount of spending is decided, as this could differ state to state and could contribute to why some have so much more to spend. 
...
joined_left %>% 
  filter(state %in% c("Alabama", "Florida", "Kentucky", "Maryland", "New Jersey", "Tennessee", "Texas", "Vermont", "Virginia", "Washington")) %>% 
  ggplot(aes(x = average_score_0_500, y = pell, color = state, group = state))+
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  geom_text(aes(label = state)) +
  xlab("Average Score: 0-500") +
  ylab("Public Pell Grant Spending, in $1,000s") +
  labs(title = "Public Pell Grant Spending by States Average Scores")
```
